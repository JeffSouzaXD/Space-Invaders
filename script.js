class Laser {
    ...
    render(context) {
       ...
       if (this.game.spriteUpdate) {
            ...
            this.game.boosArray.forEach(boss => {
...
class Player {
    ...
    update() {
        ...
        if (this.game.keys.indexOf('ArrowLeft') > -1)  { this.x -= this.speed; this.jetsFrame = 0;}
        else if (this.game.keys.indexOf('ArrowRight') > -1) { this.x += this.speed; this.jetsFrame = 2;}
        else {this.jetsFrame = 1;}
...
class Enemy {
    ...
    update(x, y) {
        ...
        // verificação de colisão entre inimigos e player
...
class Boss {
    ...
    update() {
        ...
        this.x += this.speedX;
        this.y += this.speedY;
        // colisão entre boss e projeteis
        this.game.projectilesPool.forEach(projectile => {
...
class Wave {
    ...
    render(context) {
        ...
        if (this.enemies.length <= 0) this.markedForDelete = true;
    }
    create() {
...
class Game {
    ...
    render (context, deltaTime) {
        ...
        this.player.update();
        this.boosArray.forEach(boss => {
            boss.draw(context);
            boss.update();
        })
        this.boosArray = this.boosArray.filter(object => !object.markedForDelete);
    ...
    // criando a base de munição dos projeteis
    createProjectiles() {
        for (let i = 0; i < this.numberOfProjectiles; i++) {
            this.projectilesPool.push(new Projectile());
        }
    }
    // tirando um projetil da base de munição
    getProjectile() {
        for (let i = 0; i < this.projectilesPool.length; i++) {
            if (this.projectilesPool[i].free) return this.projectilesPool[i];
        }
    }
    //area de colisão entre balas e inimigos
    checkCollision(a, b) {
        return (
            a.x < b.x + b.width &&
            a.x + a.width > b.x &&
    ...
    drawStatusText(context) {
        ...
        //loop para criar as vidas em forma de quadrado ou barra
    ...
    newWave() {
        ...
        if (this.waveCount % 3 === 0) {
            this.boosArray.push(new Boss(this, this.bossLives));
        } else {
        ...
        this.waves = this.waves.filter(object => !object.markedForDelete);
    }
    restart() {
        this.player.restart();
        this.columns = 2;
